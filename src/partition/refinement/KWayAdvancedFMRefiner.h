...
#ifndef SRC_PARTITION_REFINEMENT_KWAYFADVANCEDMREFINER_H_
#define SRC_PARTITION_REFINEMENT_KWAYADVANCEDFMREFINER_H_
...
class KWayAdvancedFMRefiner final : public IRefiner,
  ...
  static const bool dbg_refinement_kway_advanced_locked_hes = false;
  static const bool dbg_refinement_kway_advanced_infeasible_moves = false;
 ...
 public:
  KWayAdvancedFMRefiner(Hypergraph& hypergraph, const Configuration& config) noexcept :
  ...
  virtual ~KWayAdvancedFMRefiner() { }

  KWayAdvancedFMRefiner(const KWayAdvancedFMRefiner&) = delete;
  KWayAdvancedFMRefiner& operator= (const KWayAdvancedFMRefiner&) = delete;

  KWayAdvancedFMRefiner(KWayAdvancedFMRefiner&&) = delete;
  KWayAdvancedFMRefiner& operator= (KWayAdvancedFMRefiner&&) = delete;
 ...
 private:
  ...
  bool refineImpl(std::vector<HypernodeID>& refinement_nodes, const size_t num_refinement_nodes,
    ...
    while (!_pq.empty() && !_stopping_policy.searchShouldStop(num_moves_since_last_improvement,
      ...
      ASSERT(std::fabs(max_gain - gainInducedByHypergraph(max_gain_node, to_part)) < EPS, "Inconsistent gain caculation!" 
    ...
    DBG(dbg_refinement_kway_advanced_fm_stopping_crit, "KWayFM performed " << num_moves
  ...
  void deltaGainUpdates(const HypernodeID pin, const PartitionID from_part,
    ...
    if(part != from_part && part != to_part) {
      AdvancedGain delta_gain = (static_cast<double>(he_weight)/(static_cast<double>(he_size)-1.0));
      if(_pq_contains[pin * _config.partition.k + from_part]) {
	updatePin(pin,from_part,he,-delta_gain,max_allowed_part_weight);
      }
      if(_pq_contains[pin * _config.partition.k + to_part]) {
	updatePin(pin,to_part,he,delta_gain,max_allowed_part_weight);
      }
    }
    else {
      AdvancedGain delta_gain = (part == from_part ? 1.0 : -1.0)*
				(static_cast<double>(he_weight)
				/(static_cast<double>(he_size)-1.0));	
      for(PartitionID connectedPart = 0; connectedPart < _config.partition.k; ++connectedPart) {
	if(_pq_contains[pin * _config.partition.k + connectedPart]) {
	  updatePin(pin, connectedPart,he,
		    (((connectedPart == to_part 
		    || connectedPart == from_part) ? 2.0 : 1.0)*delta_gain),
            ...
            if (!_hg.isBorderNode(pin)) {
              ...
              for (const PartitionID part : _hg.connectivitySet(he)) {
                ...
                if (_pq.contains(pin, part)) {
                  ...
                  const AdvancedGain expected_gain = gainInducedByHypergraph(pin, part);
                  valid = std::fabs(_pq.key(pin, part) - expected_gain) < EPS;
                  if (!valid) {
  ...
  void updatePin(const HypernodeID pin, const PartitionID part, const HyperedgeID he,
                 const AdvancedGain delta, const HypernodeWeight max_allowed_part_weight) noexcept {
    ONLYDEBUG(he);
    ...
    if (_pq_contains[pin * _config.partition.k + part] && _already_processed_part.get(pin) != part) {
      ...
      DBG(dbg_refinement_kway_advanced_fm_gain_update,
  ...
  AdvancedGain gainInducedByHyperedge(const HypernodeID hn, const HyperedgeID he, 
				      const PartitionID target_part) const noexcept {
    const PartitionID source_part = _hg.partID(hn);
    double pin_count_from_part = static_cast<double>(_hg.pinCountInPart(he,source_part));
    double pin_count_to_part = static_cast<double>(_hg.pinCountInPart(he,target_part));
    AdvancedGain gain = static_cast<double>(_hg.edgeWeight(he))*
				 ((1.0 + pin_count_to_part - pin_count_from_part)
				  /static_cast<double>(_hg.edgeSize(he)-1));
    return gain;
  }
  
  AdvancedGain gainInducedByHypergraph(const HypernodeID hn, const PartitionID target_part) const noexcept {
    const PartitionID source_part = _hg.partID(hn);
  ...
  void insertHNintoPQ(const HypernodeID hn, const HypernodeWeight max_allowed_part_weight) noexcept {
    ...
    ASSERT([&]() {
        for (AdvancedGain gain : _tmp_gains) {
          if (gain != 0) {
            return false;
          }
        }
    ...
    for (const PartitionID target_part : _tmp_target_parts) {
      ...
      ASSERT(std::fabs(_tmp_gains[target_part]-gainInducedByHypergraph(hn,target_part)) < EPS,
	     "Gain calculation failed! Should be " << V(gainInducedByHypergraph(hn,target_part)) 
	     << " but currently it is " << V(_tmp_gains[target_part]));
      DBG(dbg_refinement_kway_advanced_fm_gain_comp, "inserting HN " << hn << " with gain "
  ...
  std::vector<AdvancedGain> _tmp_gains;
...
const PartitionID KWayAdvancedFMRefiner<T, U>::kFree;
#pragma GCC diagnostic pop
}  // namespace partition
#endif  // SRC_PARTITION_REFINEMENT_KWAYADVANCEDFMREFINER_H_
