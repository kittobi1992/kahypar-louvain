...
#ifndef SRC_PARTITION_REFINEMENT_TWOWAYADVANCEDCONNECTIVITYFMREFINER_H_
#define SRC_PARTITION_REFINEMENT_TWOWAYADVANCEDCONNECTIVITYFMREFINER_H_
...
namespace partition {
static const bool dbg_refinement_2way_advanced_connectivity_fm_improvements_cut = false;
static const bool dbg_refinement_2way_advanced_connectivity_fm_improvements_balance = false;
static const bool dbg_refinement_2way_advanced_connectivity_fm_stopping_crit = false;
static const bool dbg_refinement_2way_advanced_connectivity_fm_gain_update = false;
static const bool dbg_refinement_2way_advanced_connectivity_fm_activation = false;
static const bool dbg_refinement_2way_advanced_connectivity_locked_hes = false;

#pragma GCC diagnostic push
#pragma GCC diagnostic ignored "-Weffc++"
template <class StoppingPolicy = Mandatory,
          class FMImprovementPolicy = CutDecreasedOrInfeasibleImbalanceDecreased>
class TwoWayAdvancedConnectivityFMRefiner final : public IRefiner,
 ...
 public:
  TwoWayAdvancedConnectivityFMRefiner(Hypergraph& hypergraph, const Configuration& config) noexcept :
  ...
  virtual ~TwoWayAdvancedConnectivityFMRefiner() { }

  TwoWayAdvancedConnectivityFMRefiner(const TwoWayAdvancedConnectivityFMRefiner&) = delete;
  TwoWayAdvancedConnectivityFMRefiner& operator= (const TwoWayAdvancedConnectivityFMRefiner&) = delete;

  TwoWayAdvancedConnectivityFMRefiner(TwoWayAdvancedConnectivityFMRefiner&&) = delete;
  TwoWayAdvancedConnectivityFMRefiner& operator= (TwoWayAdvancedConnectivityFMRefiner&&) = delete;
  ...
  void activate(const HypernodeID hn,
    ...
    if (_hg.isBorderNode(hn)) {
      ...
      DBG(dbg_refinement_2way_advanced_connectivity_fm_activation, "inserting HN " << hn << " with gain " << computeGain(hn)
 ...
 private:
   ...
   for (size_t i = 0; i < num_refinement_nodes; ++i) {
	...
	_pq.disablePart(to_part);
      ...
      if (improved_cut_within_balance || improved_balance_less_equal_cut) {
        DBG(dbg_refinement_2way_advanced_connectivity_fm_improvements_balance && max_gain == 0,
            "2WayFM improved balance between " << from_part << " and " << to_part
            << "(max_gain=" << max_gain << ")");
        DBG(dbg_refinement_2way_advanced_connectivity_fm_improvements_cut && current_cut < best_cut,
    ...
    DBG(dbg_refinement_2way_advanced_connectivity_fm_stopping_crit, "KWayFM performed " << num_moves
  ...
  Gain updateNeighbours(const HypernodeID moved_hn, const PartitionID from_part,
    ...
    delta_moved_hn = temp - computeGain(moved_hn);
    ...
    for (const HypernodeID hn : _non_border_hns_to_remove) {
      DBG(dbg_refinement_2way_advanced_connectivity_fm_gain_update, "TwoWayFM deleting pin " << hn << " from PQ "
          << to_part);
      ASSERT([&]() {
          if (hn != moved_hn) {
  ...
  void fullUpdate(const PartitionID from_part,
    ...
    const HypernodeID pins_in_source_part_before = _hg.pinCountInPart(he,from_part)+1;
    const HypernodeID pins_in_target_part_before = _hg.pinCountInPart(he,to_part)-1;
    PartitionID connectivity_before = _hg.connectivity(he);
    if(_hg.connectivity(he) == 1 && pins_in_source_part_before == 1) {
      connectivity_before++;
    } else if(_hg.connectivity(he) > 1 && pins_in_target_part_before == 0) {
      connectivity_before--;
    ...
    for(HypernodeID pin : _hg.pins(he)) {
      ...
      const HypernodeID pins_in_source_part = (_hg.partID(pin) == from_part ? pins_in_source_part_before 
									: pins_in_target_part_before);
      const HypernodeID pins_in_target_part = (_hg.partID(pin) == from_part ? pins_in_target_part_before 
									: pins_in_source_part_before);
      
      if(_hg.connectivity(he) > connectivity_before) {
	delta_update -= calculateGainForNet(pins_in_source_part,pins_in_target_part,
					    _hg.edgeWeight(he),_hg.edgeSize(he),_hg.connectivity(he));
      } else if(_hg.connectivity(he) < connectivity_before) {
	delta_update += calculateGainForNet(pins_in_source_part,pins_in_target_part,
					    _hg.edgeWeight(he),_hg.edgeSize(he),connectivity_before);
      }
      delta_update += (_hg.partID(pin) == from_part ? 1.0 : -1.0)*
				    (2.0*(he_weight/(static_cast<double>(_hg.connectivity(he))*(he_size-1.0))));
      
      performNonZeroFullUpdate(pin,delta_update,num_active_pins);
    }
  ...
  void updatePin(const HypernodeID pin, const AdvancedGain gain_delta) noexcept __attribute__ ((always_inline)) {
    ...
    DBG(dbg_refinement_2way_advanced_connectivity_fm_gain_update, "TwoWayFM updating gain of HN " << pin
  ...
  AdvancedGain computeGain(const HypernodeID hn) const noexcept {
    ...
    for (const HyperedgeID he : _hg.incidentEdges(hn)) {
      ...
      gain += calculateGainForNet(_hg.pinCountInPart(he,from),_hg.pinCountInPart(he,to), 
				  _hg.edgeWeight(he), _hg.edgeSize(he), _hg.connectivity(he));
    }
    return gain;
  }
  
  AdvancedGain calculateGainForNet(HypernodeID pins_in_from_part, HypernodeID pins_in_to_part, 
				  HyperedgeWeight edge_weight, HypernodeID edge_size, 
				  PartitionID connectivity) const noexcept {
    return (static_cast<double>(edge_weight)/static_cast<double>(connectivity))
	    * ((1.0 + static_cast<double>(pins_in_to_part)-static_cast<double>(pins_in_from_part))
...
const char TwoWayAdvancedConnectivityFMRefiner<T, U>::kFree;
...
#endif  // SRC_PARTITION_REFINEMENT_TWOWAYADVANCEDCONNECTIVITYFMREFINER_H_
